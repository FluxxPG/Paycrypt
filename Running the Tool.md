...
Merchant opens the Vercel frontend.
Frontend calls the CloudFront HTTPS API endpoint.
API authenticates the merchant with JWT access tokens and refresh-token cookies.
Dashboard APIs read metrics, payments, transactions, wallets, API keys, subscriptions, and webhook logs from Supabase.
Realtime payment state changes arrive over Socket.IO through CloudFront and Redis pub/sub.
Hosted checkout flow
Merchant creates a payment intent or payment link through dashboard APIs or API keys.
If the request omits settlementCurrency and network, API resolves the merchant's saved default checkout route first.
API stores the payment, pricing quote, network choice, expiry, wallet route, and hosted-checkout metadata.
Buyer lands on /pay/[id] and sees supported network choices for that payment, QR code, wallet address, timer, and live state.
Worker services observe custody and chain activity.
Payment state progresses through payment.created, payment.pending, payment.confirmed, or payment.failed.
Webhook jobs and settlement jobs are queued and processed asynchronously.
Admin control flow
Super admin logs in through the dedicated admin login.
Admin dashboard manages merchants, wallet eligibility, subscriptions, revenue views, risk views, API keys, and operational status.
Non-custodial access is feature-gated per merchant and controlled centrally.
Core feature map
Authentication
...
Ensure Redis 6 or Redis 7 is running locally.
...
Bootstrap the EC2 host with Docker, Node, and Git.
Copy the repository and production .env to /opt/paycrypt.
Run docker compose up -d --build.
Place CloudFront in front of the EC2 API and WebSocket ports.
Point the frontend to the CloudFront distribution URL.
